...
class MjLidarTi:
    def __init__(
        ...
        self._valid_count = ti.field(dtype=ti.i32, shape=())
        self._hit_points = None
        self._valid_hit_points = None
    ...
    def trace_rays_from_dirs(self, pose_4x4: np.ndarray, local_dirs_ti: ti.ndarray):
        n_rays = local_dirs_ti.shape[0]
        self._ensure_capacity(n_rays)
        rot_np = pose_4x4[:3, :3].astype(np.float32)
        origin_np = pose_4x4[:3, 3].astype(np.float32)
        ...
        origin_ti.from_numpy(origin_np)
        self._reset_overflow()
        self._trace_dirs_kernel(
            rot_ti, origin_ti, local_dirs_ti, n_rays, self._hit_points, self._distances
        )
        ti.sync()

    def trace_valid_points(
        self,
        pose_4x4: np.ndarray,
        theta_ti: ti.ndarray,
        phi_ti: ti.ndarray,
        ...
        self._trace_valid_points_kernel(
            rot_ti,
            origin_ti,
            theta_ti,
            phi_ti,
            n_rays,
            float(min_range),
            self._valid_hit_points,
        )
        ti.sync()
        valid_count = int(self._valid_count[None])
        if valid_count <= 0:
            return np.zeros((0, 3), dtype=np.float32)
        return self._valid_hit_points.to_numpy()[:valid_count]

    def trace_valid_points_from_dirs(
        self,
        pose_4x4: np.ndarray,
        local_dirs_ti: ti.ndarray,
        min_range: float,
    ) -> np.ndarray:
        n_rays = local_dirs_ti.shape[0]
        self._ensure_capacity(n_rays)
    ...
    def _ensure_capacity(self, n_rays: int):
        if self._hit_points is None or self._hit_points.shape[0] != n_rays:
            ...
            self._valid_hit_points = ti.Vector.field(3, dtype=ti.f32, shape=n_rays)
    ...
    @ti.kernel
    def _trace_dirs_kernel(
        self,
        rot: ti.types.ndarray(dtype=ti.f32, ndim=2),
        origin: ti.types.ndarray(dtype=ti.f32, ndim=1),
        local_dirs: ti.types.ndarray(dtype=ti.f32, ndim=2),
        n_rays: ti.i32,
        hit_pts: ti.template(),
        distances: ti.template(),
    ):
        o = ti.Vector([origin[0], origin[1], origin[2]])
        for i in ti.ndrange(n_rays):
            dir_local = ti.Vector([local_dirs[i, 0], local_dirs[i, 1], local_dirs[i, 2]])
            ray_dir = ti.Vector(
                [
                    rot[0, 0] * dir_local.x + rot[0, 1] * dir_local.y + rot[0, 2] * dir_local.z,
                    rot[1, 0] * dir_local.x + rot[1, 1] * dir_local.y + rot[1, 2] * dir_local.z,
    ...
    @ti.kernel
    def _trace_valid_points_kernel(
        self,
        rot: ti.types.ndarray(dtype=ti.f32, ndim=2),
        origin: ti.types.ndarray(dtype=ti.f32, ndim=1),
        theta_arr: ti.types.ndarray(dtype=ti.f32, ndim=1),
        ...
        for i in ti.ndrange(n_rays):
            ...
            if best_t >= min_range and best_t < self._cutoff:
                valid_index = ti.atomic_add(self._valid_count[None], 1)
                valid_hit_pts[valid_index] = ti.Vector(
                    [best_t * dir_local.x, best_t * dir_local.y, best_t * dir_local.z]
                )

    @ti.kernel
    def _trace_valid_dirs_kernel(
        self,
        rot: ti.types.ndarray(dtype=ti.f32, ndim=2),
        origin: ti.types.ndarray(dtype=ti.f32, ndim=1),
        local_dirs: ti.types.ndarray(dtype=ti.f32, ndim=2),
